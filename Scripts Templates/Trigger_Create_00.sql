-----------------------------------------------------------------------------
-- Create trigger <trigger_name, sysname, TR_tablename_IUD> on table <table_name, sysname, tbl>.
-----------------------------------------------------------------------------
...
BEGIN
    ...
    SELECT @nMyErr = @@ERROR, @nRowsAffected = @@ROWCOUNT  IF @nMyErr != 0  BEGIN  SET CONTEXT_INFO 0x0  SET @sMsg = N'   *** Failed to create trigger <trigger_name, sysname, TR_tablename_IUD> - @@ERROR: ' + CAST(@nMyErr AS NVARCHAR)   RAISERROR(@sMsg, 16, 1)   IF 0 < @@TRANCOUNT ROLLBACK   RETURN  END
END
...
CREATE TRIGGER <trigger_name, sysname, TR_tablename_IUD>
     ON <schema_name, sysname, dbo>.<table_name, sysname, tbl>
     
 AFTER <trigger_after, , INSERT, UPDATE, DELETE>
     AS
